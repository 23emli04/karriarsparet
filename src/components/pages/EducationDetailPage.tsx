...
import type { EducationEvent } from "../../types/EducationEvent";
...
export default function EducationDetailPage() {
  ...
  const [educationEvent, setEducationEvent] = useState<EducationEvent | null>(null);
  ...
  useEffect(() => {
    ...
    const eventId = education.id.startsWith("i")
        ? "e" + education.id.slice(1)
        : education.id;
    ...
    educationsApi
        ...
        .then((event) => {
          if (isMounted) setEducationEvent(event);
        })
        .catch((err) => {
          console.error("Failed to fetch education event", err);
          if (isMounted) setEducationEvent(null);
        });
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <article className="rounded-2xl p-6 sm:p-8 bg-white shadow-sm border border-slate-200/80">
            <div className="flex flex-wrap items-center gap-2 mb-4">
              <span className="inline-flex text-xs font-semibold uppercase tracking-wider  bg-blue px-2.5 py-1 rounded-md">
                {education.educationLevel.code}

              </span>
              {education.distance && (
                  <span className="inline-flex text-xs font-medium text-slate-600 bg-slate-100 px-2.5 py-1 rounded-md">Distans</span>
              )}
              {education.resultIsDegree && (
                  <span className="inline-flex text-xs font-medium text-slate-600 bg-slate-100 px-2.5 py-1 rounded-md">Examen</span>
              )}
            </div>
            ...
            {education.code && <p className="text-sm text-slate-500 mb-6">Kod: {education.code}</p>}
           <p className="mb-3"> Länk till hemsida:<a target="_blank" href={educationEvent?.urlSwe} className="text-blue-400"> {educationEvent?.urlSwe}</a></p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8 pb-8 border-b border-slate-100">
              <DetailRow label="Studietakt" value={pace.length ? pace.map((p) => `${p}%`).join(", ") : null} />
              <DetailRow label="Språk" value={langs.length ? langs.join(", ") : null} />
              <DetailRow label="Högskolepoäng" value={education.credits != null ? String(education.credits) : null} />
              <DetailRow label="Studietid" value={timeOfStudy.length ? timeOfStudy.join(", ") : null} />
              <DetailRow label="Upphör" value={education.expires ? formatDate(education.expires) : null} />
            </div>
            ...
            {executions.length > 0 && (
                <DetailSection title="Löpande period">
                  <ul className="space-y-2">
                    {executions.map((ex, i) => (
                        <li key={i} className="text-slate-800">{formatDate(ex.start)} – {formatDate(ex.end)}</li>
                    ))}
                  </ul>
                </DetailSection>
            )}
            ...
            {educationEvent && (
                <DetailSection title="Evenemang">
                  <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                    <h3 className="font-medium text-slate-900 mb-2">{educationEvent.title}</h3>
                    {educationEvent.executionStart && educationEvent.executionEnd && (
                        <p className="text-sm text-slate-600 mb-2">
                          {formatDate(educationEvent.executionStart)} – {formatDate(educationEvent.executionEnd)}
                        </p>
                    )}
                    {educationEvent.applicationLast && (
                        <p className="text-sm text-slate-600">
                          Sista ansökningsdag: {formatDate(educationEvent.applicationLast)}
                        </p>
                    )}
                    {educationEvent.fee && (
                        <p className="text-sm text-slate-600">Avgift: {educationEvent.fee}</p>
                    )}
                  </div>
                </DetailSection>
            )}
            ...
            {(eligibility?.eligibilityDescription || eligibilityTags.length > 0) && (
                <div className="mt-8">
                  <DetailSection title="Behörighet">
                    {eligibility?.eligibilityDescription && <p className="text-slate-800">{eligibility.eligibilityDescription}</p>}
                    {eligibilityTags.length > 0 && (
                        <div className="flex flex-wrap gap-2 mt-2">
                          {eligibilityTags.map((tag) => (
                              <span key={tag} className="inline-flex text-xs font-medium text-slate-600 bg-slate-100 px-2 py-1 rounded">{tag}</span>
                          ))}
                        </div>
                    )}
                  </DetailSection>
                ...
                <div className="mt-8">
                  <DetailSection title="Ämnen">
                    <ul className="space-y-1">
                      {subjects.map((s) => (
                          <li key={s.code} className="text-slate-800">{s.name}</li>
                      ))}
                    </ul>
                  </DetailSection>
                </div>
                ...
                <div className="mt-8 pt-8 border-t border-slate-100 space-y-6">
                      ...
                      <DetailSection title="Kompetenser">
                        <div className="flex flex-wrap gap-2">
                          {enrichedCompetencies.map((c) => (
                              <span key={c} className="inline-flex text-sm text-slate-700 bg-amber-50/80 text-amber-800 px-2.5 py-1 rounded-md">{c}</span>
                          ))}
                        </div>
                      </DetailSection>
                  )}
                  {enrichedGeos.length > 0 && (
                      <DetailSection title="Platser">
                        <div className="flex flex-wrap gap-2">
                          {enrichedGeos.map((g) => (
                              <span key={g} className="inline-flex text-sm text-slate-700 bg-slate-100 px-2.5 py-1 rounded-md">{g}</span>
                          ))}
                        </div>
                      </DetailSection>
                  )}
                      ...
                      <DetailSection title="Yrken">
                        <div className="flex flex-wrap gap-2">
                          {enrichedOccupations.map((o) => (
                              <span key={o} className="inline-flex text-sm text-slate-700 bg-slate-100 px-2.5 py-1 rounded-md">{o}</span>
                          ))}
                        </div>
                      </DetailSection>
                  )}
